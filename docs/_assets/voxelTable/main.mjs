...
function createTableRow(option, ...datas) {
    ...
    for (const index in datas) {
        ...
        if (option.width && typeof option.width[index] == "number" && option.width[index] >= 0 && option.width[index] <= 100) {
...
function build() {
    console.log('方块速查表 v3');
    let localVoxelTypes = localStorage.getItem('seenVoxels');
    while (seenVoxels.length > 0)
        seenVoxels.pop();
    if (localVoxelTypes !== null) {
        seenVoxels.push(...JSON.parse(localVoxelTypes));
    }
    ...
    if (el instanceof HTMLElement) {
        ...
        for (let width of [25, 15, 15, 35, 30]) {
        ...
        for (const voxelName of voxelNames) {
            ...
            let description = voxelTypes_zhcn[voxelName] ?? voxelName,
                ...
                color,
                classList = [['', 'old-only', 'pro-only', ''][_0x10]];
            ...
            if (seenVoxels.length > 0 && !seenVoxels.includes(voxelName)) {
...
function save(voxelNames) {
    try {
        localStorage.setItem('seenVoxels', JSON.stringify(voxelNames));
    } catch (e) {
